...
public class BankMenuTest {
    ...
    public void testWithdrawMenu() {
        ...
        bankAccount = new BankAccount("BankAccountTest", "1");
        Scanner sc = mock(Scanner.class);
        bankMenu = new BankMenu(bankService, bankAccount, sc);
        double amountWithdraw = 500.0;
        when(sc.next()).thenReturn("c", "e");
        when(sc.nextDouble()).thenReturn(500.0);
        bankMenu.menu();
    ...
    public void testDepositMenu() {
        ...
        Scanner sc = mock(Scanner.class);
        bankMenu = new BankMenu(bankService, bankAccount, sc);
        when(sc.next()).thenReturn("b", "e");
        when(sc.nextDouble()).thenReturn(500.0);
        bankMenu.menu();
        verify(bankService, atLeast(1)).deposit(bankAccount, 500.0);
    }
